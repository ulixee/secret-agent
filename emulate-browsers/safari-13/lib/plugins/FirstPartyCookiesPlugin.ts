import { URL } from 'url';
import BrowserEmulatorPlugin from '@secret-agent/browser-emulator-utils/lib/BrowserEmulatorPlugin';
import {
  canonicalDomain,
  Cookie,
  CookieJar,
...
import { DomOverridesBuilder } from '@secret-agent/browser-emulator-utils';

export default class FirstPartyCookiesPlugin extends BrowserEmulatorPlugin {
  ...
  public onLoadUserProfileCookies(cookies, storage) {
  ...
  public async beforeHttpResponse(resource: IHttpResourceLoadDetails): Promise<any> {
    ...
    for (const setCookie of cookies) {
      try {
        ...
        this.log.warn<any>('Could not set cookie', { error });
      }
    }
  }
  ...
  public onNewPuppetPage(page: IPuppetPage): Promise<any> {
    ...
    const callback = async ({ cookie, origin }) => {
      try {
        ...
        this.log.warn<any>('Error setting cookie from page', { error });
      }
    };
  ...
  private loadProfileCookies(cookies, storage) {
  ...
  private isMinimumVersion(minor: number, patch = 0) {
    const userAgentVersion = this.userAgentOption.version;
    return Number(userAgentVersion.minor) >= minor && Number(userAgentVersion.patch) >= patch;
  }
}
